...
public class Ipv6RoutingComponent {
    ...
    private final HostListener hostListener = new InternalHostListener(); //la aplicacion escucha como es la topologia
    ...
    @Activate //lo primero que se ejecuta al arrancar la aplicacion
    protected void activate() {
        appId = mainComponent.getAppId();

        hostService.addListener(hostListener); //inicializar listeners
        linkService.addListener(linkListener);
        deviceService.addListener(deviceListener);

        // Schedule set up for all devices.
        mainComponent.scheduleTask(this::setUpAllDevices, INITIAL_SETUP_DELAY); // se llama a esa funcion
